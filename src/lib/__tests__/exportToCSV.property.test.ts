...
import { exportFeedbackToCSV } from '../exportToCSV';
import type { FeedbackResponse, FeedbackForm } from '../../types';

describe('Property: CSV Export filename includes current date', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('filename matches feedback-responses-YYYY-MM-DD.csv for any date', () => {
    ...
    fc.assert(
      fc.property(dateArb, (date) => {
        ...
        exportFeedbackToCSV(responses, forms);
        ...
        const expectedFilename = `feedback-responses-${yyyy}-${mm}-${dd}.csv`;
