...
| `/api/import-employees` | POST | Firebase ID token | Accepts bulk employee data from CSV or Excel files and writes it to the `employees` collection scoped to the authenticated user's tenant. |
| `/api/employees` | GET | Firebase ID token | Returns all employee records for the caller's tenant, sorted by name. Tenant is read from the `x-tenant-id` header (injected by middleware) with a fallback to the token's `tenantId` claim. |
| `/api/forms/lookup` | GET | None | Resolves a form by document ID or slug using the Admin SDK (bypasses client-side Firestore rules and composite-index requirements). Accepts `?idOrSlug=` and optional `?tenantId=`. Used by `FeedbackPageClient` for slug-based public form URLs. |
...
| `FeedbackFormsList` | Filterable list of all tenant forms with view, edit, toggle, and delete actions per form card. Staff users see only their own forms. |
| `FilterSortBar` | Generic reusable filter bar with text search, status pills, date range picker, sort field/direction selector, and optional form selector dropdown. |
...
| `EmployeeManagementSection` | Full employee directory UI embedded in the Settings page. Supports search/pagination, individual add/edit/delete with confirmation modal, and bulk import from CSV or Excel (`.xlsx`, `.xls`). Calls `bulkSaveEmployees()` for imports. |
| `EmployeeSelector` | Multi-select employee picker used in `PollBuilder` for choosing Staff Nomination nominees. Renders a searchable list of employees from the tenant directory and emits an array of selected Employee IDs. |
| `PollBuilder` | Two-step poll creation form (Details → Questions/Nominees). Handles both opinion poll and staff nomination types. Validates nominee Employee IDs against the tenant directory before saving. Calls `createPoll()` from `pollsFirestore.ts`. |
| `PollResultsDisplay` | Results visualisation for a single poll. Shows per-option vote counts and percentages as bar charts. For staff nominations, highlights the leading nominee. Reads live data from `usePollResults`. |
| `PollShareButton` | Renders a modal with the poll's public voting URL, a QR code, and share shortcuts (WhatsApp icon, Email icon). Used on the polls list and results pages. |
| `PollStatusToggle` | Toggle button that activates or deactivates a poll (`isActive` field). Calls `updatePoll()` and shows an inline loading state. |
| `PollVotingInterface` | Public-facing voting UI rendered at `/poll/[pollId]`. Handles both opinion and staff nomination question types, duplicate-vote detection (localStorage + voterUid check), and submission via `submitPollResponse()`. Shows results after voting if `showResults` is set to `"after_voting"` or `"always"`. |
| `RichTextEditor` | Tiptap-based rich text editor used for form descriptions and section descriptions. Supports bold, italic, bullet lists, ordered lists, and links. Outputs HTML. Sanitized with DOMPurify before storage and before rendering on the public form. |
...
| `usePollResults` | `{ results: PollVote[], loading }` | Subscribes to the `poll-votes` collection via Firestore `onSnapshot` for a given `pollId` and `tenantId`. Returns live vote data that updates in real time as votes arrive. Used by `PollResultsDisplay`. |
| `useStableRole` | `{ role, tenantId, uid, isLoading, error }` | Resolves the current user's Firebase Auth custom claims (`role`, `tenantId`) with exponential-backoff retry (up to 3 attempts: 1 s, 2 s, 4 s) and a Firestore `tenant-admins` fallback if claims remain missing after retries. Provides a stable role value that survives token refresh cycles. |
...
| `firestore.ts` | Core Firestore CRUD helpers for feedback forms and responses: `saveForm`, `updateForm`, `deleteForm` (cascades to responses), `getAllForms` (supports `createdBy` filter for staff scoping), `getAllResponses`, `submitFeedback`, `getFormById`, `getFormBySlug`, `isSlugTaken`, `hasIpSubmittedForm`, `incrementFormCount`. All queries include `tenantId` filtering. |
...
| `employeesFirestore.ts` | Employee CRUD: `getAllEmployees`, `saveEmployee`, `updateEmployee`, `deleteEmployee`, `bulkSaveEmployees`, `seedEmployeesIfEmpty`. All operations scoped to `tenantId`. Document IDs use the pattern `{tenantId}_{Employee ID}`. |
| `pollsFirestore.ts` | Poll CRUD and voting: `getAllPolls`, `getPollById`, `createPoll`, `updatePoll`, `deletePoll`, `submitPollResponse` (batch-writes a `poll-responses` doc and one `poll-votes` doc per question), `validateEmployeeNominees`. All operations scoped to `tenantId`. |
| `pollsExport.ts` | Exports raw poll vote data to `.xlsx`. `exportPollResultsToExcel(poll, votes)` flattens `PollVote` documents into rows with poll title, type, question ID, selected option, voter UID, voter employee ID, and submission timestamp. Filename: `poll-results-{pollId}-{YYYY-MM-DD}.xlsx`. |
...
| `exportToCSV.ts` | Builds and triggers a `.csv` download. `exportFeedbackToCSV(responses, forms)` serialises the current filtered response set to comma-separated values. |
...
| `TenantAdmin` | User-to-organisation mapping: `uid`, `tenantId`, `email`, `role` (`"owner"` \| `"staff"`), `formCount`, `formLimit`, `invitedBy`, `notificationEmails`, `welcomeSent`, `photoUrl` |
| `TenantRole` | `"owner"` \| `"staff"` — raw Firestore/claims value. Displayed as "Admin" and "Staff" respectively in the UI. |
| `TenantFeatures` | Feature flag object: `feedbackForms`, `employeeRecords`, `seoSettings`, `hidePoweredBy`, `allowResponseSharing` |
| `FeedbackForm` | Feedback form document shape — includes `slug`, `createdBy`, `sections`, `collectName`, `stepByStep`, `ogImageUrl` |
| `FormSection` | Section grouping within a form: `id`, `name`, `description?` |
| `FeedbackQuestion` | Individual question shape within a form — includes `sectionId?` for section grouping |
...
| `Poll` | Poll document shape — includes `type` (`"opinion"` \| `"staff_nomination"`), `questions`, `nominees`, `showResults`, `allowMultipleVotes`, `endDate`, `slug`, `createdBy` |
| `PollQuestion` | Individual poll question: `id`, `question`, `type` (`"single_choice"` \| `"multiple_choice"`), `options`, `maxSelections?`, `required` |
| `PollResponse` | Poll voter submission: `pollId`, `voterUid`, `employeeId?`, `responses` map, `submittedAt`, `tenantId` |
| `PollVote` | Individual question-level vote: `pollId`, `questionId`, `optionId`, `voterUid`, `employeeId?`, `submittedAt`, `tenantId` |
...
| First-login claims propagation delay | Up to ~7 seconds in edge cases. `TenantContext` retries up to 3 times with exponential backoff (1 s, 2 s, 4 s) before falling back to a direct Firestore `tenant-admins` read. `useStableRole` applies the same logic independently for the forms page role gate. | Firebase platform behaviour — handled automatically |
